...
namespace Jibberwock.Persistence.DataAccess.Commands.Notifications
{
    ...
    public class Notify : AuditingCommand<Notification, ModifyNotification>
    {
        ...
        protected override async Task<Notification> OnAuditedExecute(IReadWriteDataSource dataSource, IDbTransaction transaction, ModifyNotification provisionalAuditTrailEntry)
        {
            ...
            var createdNotifications = await databaseConnection.QueryAsync<Notification, EmailBatch, Notification>("core.usp_CreateNotification",
                (n, eb) =>
                {
                    if (eb != null)
                    { n.EmailBatch = eb; }
            ...
            Notification.Id = resultantNotification.Id;
            if (resultantNotification.EmailBatch != null)
            {
                Notification.EmailBatch = new EmailBatch() { Id = resultantNotification.EmailBatch.Id };
